...
function NetworkedUsersPage() {
  ...
  const [draft, setDraft] = useState({ name: "", email: "", role: "Logistic Broker", region: REGIONS[0].name });

  const filtered = list.filter((u) => {
    if (region && u.region !== region) return false;
    if (status && u.status !== status) return false;
    if (q && !`${u.name} ${u.email} ${u.id} ${u.role}`.toLowerCase().includes(q.toLowerCase())) return false;
    return true;
  });
  ...
  const add = () => {
    ...
    setList([{
      id, name: draft.name, email: draft.email, region: draft.region, kind: "Networked",
      role: draft.role, status: "Pending", joined: new Date().toISOString().slice(0, 10), avatar: initials,
      phone: "+00 000 0000", walletBalance: 0, rating: 5, trips: 0
    }, ...list]);
    toast.success(`Networked User ${draft.name} added as ${draft.role}`);
    setOpen(false); setDraft({ name: "", email: "", role: "Logistic Broker", region: REGIONS[0].name });
  };
    ...
    <AdminLayout>
      ...
      <div className="flex flex-wrap gap-2 mb-4">
        <div className="relative flex-1 min-w-[220px]">
          ...
          <Input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Search name, email, role, ID…" className="w-full pl-9" />
        </div>
      ...
      <Table>
        <THead><TR><TH>User</TH><TH>Role</TH><TH>Email</TH><TH>Region</TH><TH>Status</TH><TH>Joined</TH></TR></THead>
        <tbody>
          {filtered.map((u) => (
            <TR key={u.id}>
